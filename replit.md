...
Implements a Super Agent (orchestrator) and specialized Agents (specialists) model. The agent creation/edit modal uses 6 tabs: Basics, Persona (Prompt Persona), Voice (Voice Persona), Prompt, Behavior, Guardrails. Settings are bifurcated:
- **Voice Persona UI** (audio/TTS properties in Voice tab): Gender, Age, Accent, Voice Tone, Emotion style, plus all technical voice/call settings
- **Prompt Persona** (behavioral properties in Persona tab): Brand Voice, Empathy level, Verbosity level, Adaptability, plus tone, style, greeting, personality
