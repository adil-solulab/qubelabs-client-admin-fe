import { Globe, MoreHorizontal, Play, Trash2, RefreshCw, CheckCircle, Clock, Loader2, ExternalLink } from 'lucide-react';
...
import type { URLSource } from '@/types/knowledgeBase';
import { TRAINING_STATUS_LABELS, SYNC_FREQUENCY_LABELS } from '@/types/knowledgeBase';
import { cn } from '@/lib/utils';

interface URLSourceCardProps {
  source: URLSource;
  onTrain: (source: URLSource) => void;
  onDelete: (source: URLSource) => void;
  onResync: (source: URLSource) => void;
  canEdit?: boolean;
  canDelete?: boolean;
}

export function URLSourceCard({ source, onTrain, onDelete, onResync, canEdit = true, canDelete = true }: URLSourceCardProps) {
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              <Badge variant="outline" className={cn('text-xs gap-1', getCrawlBadge(source.status))}>
                ...
                {source.status === 'crawling' ? 'Crawling...' : source.status === 'completed' ? 'Crawled' : source.status}
              </Badge>
            ...
            <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground">
              <span>{source.pagesDiscovered} pages</span>
