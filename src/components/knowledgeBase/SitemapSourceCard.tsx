import { Map, MoreHorizontal, Play, Trash2, RefreshCw, CheckCircle, Clock, Loader2, ExternalLink } from 'lucide-react';
...
import type { SitemapSource } from '@/types/knowledgeBase';
import { TRAINING_STATUS_LABELS, SYNC_FREQUENCY_LABELS } from '@/types/knowledgeBase';
import { cn } from '@/lib/utils';

interface SitemapSourceCardProps {
  source: SitemapSource;
  onTrain: (source: SitemapSource) => void;
  onDelete: (source: SitemapSource) => void;
  onResync: (source: SitemapSource) => void;
  canEdit?: boolean;
  canDelete?: boolean;
}

export function SitemapSourceCard({ source, onTrain, onDelete, onResync, canEdit = true, canDelete = true }: SitemapSourceCardProps) {
  ...
  const crawlPercent = source.totalUrls > 0 ? Math.round((source.crawledUrls / source.totalUrls) * 100) : 0;
      ...
      <CardContent className="p-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              <Badge variant="outline" className={cn('text-xs gap-1', getCrawlBadge(source.status))}>
                ...
                {source.status === 'crawling'
                  ? `Crawling ${source.crawledUrls}/${source.totalUrls}`
                  : source.status === 'completed'
                  ? `${source.totalUrls} pages crawled`
                  : source.status}
              </Badge>
            ...
            {source.status === 'crawling' && (
              <div className="mt-2">
                <Progress value={crawlPercent} className="h-1.5" />
              </div>
            )}
