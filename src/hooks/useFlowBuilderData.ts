    ...
    nodes: [
      ...
      },
      {
        ...
        data: {
          label: 'Safety Check',
          safetyConfig: {
            botType: 'voice' as const,
            checks: { sentimentAnalysis: true, piiDetection: true, policyViolation: true, profanityFilter: true, topicGuardrail: true },
            sentimentThreshold: 'low' as const,
            sentimentEscalateOnRepeated: true,
            sentimentRepeatCount: 3,
            piiTypes: ['credit_card', 'ssn', 'phone', 'email', 'address', 'name', 'government_id'] as ('credit_card' | 'ssn' | 'phone' | 'email' | 'address' | 'name' | 'government_id' | 'date_of_birth')[],
            policyCategories: ['harassment', 'threats', 'abuse', 'fraud'] as ('harassment' | 'threats' | 'abuse' | 'fraud' | 'scams' | 'data_leakage' | 'confidential_content')[],
            profanitySeverity: 'moderate' as const,
            profanityGraceCount: 1,
            blockedTopics: 'Medical advice\nLegal advice\nPolitics',
            onHighRisk: 'warn_then_escalate' as const,
            onMediumRisk: 'continue_with_warning' as const,
            onPiiDetected: 'mask_log_continue' as const,
            onSensitiveTopic: 'safe_fallback' as const,
            customRules: 'If user mentions refund + anger → escalate immediately\nIf user mentions cancel account + negative sentiment → route to retention agent',
            enableLogging: true,
          },
        },
        connections: [],
